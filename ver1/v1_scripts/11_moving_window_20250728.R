...
# convert into from spatraster to raster
elev <- raster(elev)
...
for(g in 1:length(tel_data)){
  ...
  # Remove the last few start times so that the last window fits entirely within the dataset, rather than being a partial window that extends beyond the available data.

  # need not do the next two lines for the times, the package does full days and not investigating any temporal specific (like time of day, day/night differences etc)
  ...
  # # Convert Unix timestamps to POSIXct ( the data is in UTC)
  ...
  covariates <- data.frame(collar_id = character(length(times)),
