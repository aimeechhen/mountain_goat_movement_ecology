...
# import data ----
load("./scripts/working/data/rsf/rsf_results_20250505.rda")
...
rsf_model <- lmer(rsf_elev_est ~ fire_year + (1|collar_id), 
...
rsf_model2 <- lmer(rsf_dist_escape_est ~ fire_year + (1|collar_id), 
                   # family =  gaussian("identity"), # ranges from -1,1
                   data = rsf_results,
                   na.action = "na.fail")
rsf_null2 <- lmer(rsf_dist_escape_est ~ 1 + (1|collar_id), 
                  # family =  gaussian("identity"), # ranges from -1,1
                  data = rsf_results,
                  na.action = "na.fail")
...
rsf_columns <- c("rsf_elev_low", "rsf_elev_est", "rsf_elev_high", "rsf_elev_cov",
                 # "rsf_elev_scaled_low", "rsf_elev_scaled_est", "rsf_elev_scaled_high", "rsf_elev_scaled_cov",
                 "rsf_dist_escape_low", "rsf_dist_escape_est", "rsf_dist_escape_high", "rsf_dist_escape_cov")#,
                 # "rsf_dist_escape_scaled_low", "rsf_dist_escape_scaled_est", "rsf_dist_escape_scaled_high", "rsf_dist_escape_scaled_cov")
...
plot_rsf_elevation <-
  ...
  geom_pointrange(aes(x = goat_name, y = rsf_elev_est, ymin = rsf_elev_low, ymax = rsf_elev_high,
                      colour = year),
...
plot_rsf_dist_esc <-
  ggplot(data = rsf_results) +
  geom_hline(yintercept = 0, col = "grey70", linetype = "dashed") +
  geom_pointrange(aes(x = goat_name, y = rsf_dist_escape_est, ymin = rsf_dist_escape_low, ymax = rsf_dist_escape_high, color = year),
                  size = 0.5, position = position_dodge(width = 0.5)) +
...
plot_rsf_annual_elev <-
  ...
  geom_pointrange(aes(x = year, y = rsf_elev_est, ymin = rsf_elev_low, ymax = rsf_elev_high, 
                      color = year), 
                  size = 0.5, position = position_dodge(width = 0.5)) +
  ylab("Elevation") +
  # ggtitle("a)") +
...
plot_rsf_annual_dist_esc <- 
  ggplot(data = rsf_yearly) +
  geom_hline(yintercept = 0, col = "black", linetype = "dashed") +
  geom_pointrange(aes(x = year, y = rsf_dist_escape_est, ymin = rsf_dist_escape_low, ymax = rsf_dist_escape_high, 
                      color = year), 
                  size = 0.5, position = position_dodge(width = 0.5)) +
  ylab("Distance to Escape Terrain") +
  # ggtitle("b)") +
...
ggsave(plot_rsf_annual,
       file="figures/rsf/rsf_annual_mean_20250923.png",
