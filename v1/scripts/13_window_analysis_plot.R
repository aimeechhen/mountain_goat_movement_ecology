...
# win_results <- read.csv("./scripts/working/data/window_analysis (old, do not use)/merged_windows_extracted_combined_data_20250317.rds")
# 
# covariates <- readRDS("./scripts/working/data/window_analysis (old, do not use)/covariates_extracted_combined_data_20250317.rds") # 2023-2025
# covariates <- readRDS("./data/window_analysis (old, do not use)/merged_windows_extracted_covariates_combined_data_20250317.rds") # does not work



library(lubridate)
dir_path <- "./data/window_analysis (old, do not use)/covariates_20250316/" # this only has fire period
dir_path <- "./scripts/working/data/window_analysis (old, do not use)/original_data/covariates_20250118/"
...
# didnt have time to run entire window analysis, had to break it up and combine for deadline
...
# Read each file and add a collar_id column before combining
...
goat_info <- read.csv("./scripts/working/data/goat_info.csv")
...
dir_path <- "./scripts/working/data/window_analysis (old, do not use)/new_data/covariates_20250219/"
# Load .shp, .tif etc files within a folder including all the subfolders, remember these are saved as df
rds_files <- list.files(dir_path, pattern = "covariates.*\\.rds$", full.names = TRUE, recursive = TRUE)
# Import/read all the files into a list
rds_list <- lapply(rds_files, readRDS)
...
goat_info$collar_id <- as.factor(goat_info$collar_id)
goat_info <- goat_info[goat_info$goat_name %in% goats,]
covariates <- covariates[covariates$goat_name %in% goats,]
covariates <- merge(covariates, goat_info[, c("collar_id","goat_name")], by = "goat_name", all.x = TRUE)
covariates <- covariates[order(covariates$collar_id, covariates$window_start), ]
covariates$goat_color <- goat_palette[match(covariates$collar_id, goats)]

goat_pal <- setNames(goat_palette, goats)
covariates$goat_color <- goat_pal[covariates$goat_name]
...
# now to go living labs r script
