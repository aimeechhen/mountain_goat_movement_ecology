...
# Check exported files and plot

fire_pixel_sf <- st_read("C:/Users/achhen/Desktop/NOAA/GOES/18/FDCF/fire_pixel_point/2023/231/2023231051022200000.shp")
pixel_area_sf <- st_read("C:/Users/achhen/Desktop/NOAA/GOES/18/FDCF/fire_pixel_boundary/2023/231/2023231051022200000.shp")
pixel_boundary_outline_sf <- st_read("C:/Users/achhen/Desktop/NOAA/GOES/18/FDCF/fire_pixel_boundary_outline/2023/231/2023231051022200000.shp")

# create a geometry that represents the min. convex geometry, i.e. draw an outline-ish
pixel_area_sf_geo <- st_convex_hull(pixel_area_sf)
area_outline_unsmoothed_sf_geo <- st_convex_hull(pixel_boundary_outline_sf)

plot(st_geometry(fire_pixel_sf), col = 'orange', pch = 20, main = "Fire pixels, fire area, fire outline")
plot(st_geometry(pixel_area_sf_geo), add = TRUE, border = 'red', lwd = 2)
plot(st_geometry(area_outline_unsmoothed_sf_geo), add = TRUE, border = 'blue', lwd = 2)


# Inspect
shp_dat <- st_read("C:/Users/achhen/Desktop/NOAA/GOES/18/FDCF/fire_pixel_boundary/2023/231/2023231051022200000.shp")

#extract file name
shp_dat$image_id_number <- gsub("\\.shp$", "", basename("C:/Users/achhen/Desktop/NOAA/GOES/18/FDCF/fire_pixel_boundary/2023/231/2023231051022200000.shp"))
...
# dat <- dat[dat$date == "2023-08-19",]
dat$timestamp = as.POSIXct(dat$timestamp, format = "%Y-%m-%d %H:%M:%S", tz = "America/Vancouver")
...
for (i in 1:nrow(dat_sf)) {
  ...
  # extract fire event within the time window
  fire_activity <- shp_dat[shp_dat$timestamp >= start_time & shp_dat$timestamp <= end_time,]
  ...
  if (nrow(fire_activity) == 0) {
    ...
    # message("No fire activity detected, moving on to the next collar point")
  ...
  fire_activity_sf <- st_as_sf(fire_activity, coords = c("longitude", "latitude"), crs = 4326, agr = "constant")
  # Match crs
  st_crs(collar_point_sf) <- st_crs(fire_activity_sf)
  
  
  # If fire points exist in the time window, proceed with calculation
  tryCatch({
    if (nrow(fire_activity) > 0) {
      # message("Fire activity detected, calculating distance")
      dat$fire_event[i] <- "yes"
      # draw an outline around the fire area
      area_outline_unsmoothed_sf_geo <- st_convex_hull(fire_activity_sf$geometry)
      # calculate the distance between the collar point and the fire point, output comes out as a matrix
      dist_matrix <- st_distance(collar_point_sf, area_outline_unsmoothed_sf_geo)
...
# write.csv(dat, "./data/input_data/20240823_dist_to_fire_boundary_dat_goes18.csv")
write.csv(dat, "./data/input_data/20240823_dist_to_fire_boundary_TEST.csv")
...
# if (nrow(fire_activity_sf) > 1) {
#   #combine polygons into one geometry
#   combine_polygons <- st_union(area_outline_unsmoothed_sf_geo$geometry)
#   #store the outline geometry
#   dat$area_outline_unsmoothed_sf_geo[i] <- combine_polygons
# } else {
#   # If time window is empty, set results to NA
#   dat$area_outline_unsmoothed_sf_geo[i] <- area_outline_unsmoothed_sf_geo$geometry
#   
# }
...
# pixel_point_sf <- st_read("C:/Users/achhen/Desktop/NOAA/GOES/18/FDCF/fire_pixel_boundary/test/233/2023233013020400000.shp")
fire_pixel_sf <- st_read("C:/Users/achhen/Desktop/NOAA/GOES/18/FDCF/fire_pixel_point/2023/231/2023231051022200000.shp")
pixel_area_sf <- st_read("C:/Users/achhen/Desktop/NOAA/GOES/18/FDCF/fire_pixel_boundary/2023/231/2023231051022200000.shp")
pixel_boundary_outline_sf <- st_read("C:/Users/achhen/Desktop/NOAA/GOES/18/FDCF/fire_pixel_boundary_outline/2023/231/2023231051022200000.shp")
...
# Combine shapefiles
rast_dat <- do.call(rbind, rast_list)
