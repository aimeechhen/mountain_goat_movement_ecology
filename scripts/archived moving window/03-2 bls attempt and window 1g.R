...
fire_goats <- fire_goats[fire_goats$date >= fire_start & fire_goats$date <= fire_end, ]
...
dat = plyr::rename(fire_goats, c('latitude' = 'location.lat',
...
#create folder
dir.create(paste0("./data/bls/fire_goat", "/bls_20250201"), recursive = TRUE, showWarnings = TRUE)

# Set results directory
# bls_dir = paste0(getwd(), "/data/bls/fire_goat", '/bls_20241230')
bls_dir = paste0(getwd(), "/data/bls/fire_goat", '/bls_20250201')
...
get_bls <- function(DATA) {
  ...
  tryCatch({
    ...
    RESULTS <- data.frame(individual.local.identifier = name1,
                          timestamp = TIMESTAMP)
    ...
    for(i in 1:length(times)) {
      ...
      SUBSET <- combine_data[times[i] <= combine_data$t & combine_data$t <= times[i] + win,] # +win means window size i.e. 3 days
      ...
      print(paste((i),"of",length(times),"iterations. Window segment:", 
                  ...
                  "for mountain goat:", name1))
      
      # correct for shortest time interval
      #Create a new timestamp column -> round down to the nearest minute via format() and drop the seconds in the timestamp
      combine_data$timestamp_rs <- as.POSIXct(format(combine_data$timestamp, "%Y-%m-%d %H:%M"))
      ...
      tryCatch({
        if (nrow(SUBSET) > 0) {
          ...
          start <- combine_data$timestamp_rs[1]
          stop <- tail(combine_data,1)$timestamp_rs
        ...
        bls <- rollapply(data = ind1, width = 3,  #choose the window size 
    ...
    tryCatch({
      if (nrow(SUBSET) > 0) {
        ...
        if (inherits(FITS, "ctmm")) {
          ...
          fitsum <- summary(FITS, units = FALSE)
          
          # store output in a list for future extraction to review summary output
          fitsum_list[[i]] <- summary(FITS, units = FALSE)
          
          
          
          ## 1. Get movement model that was best fit ----
          model_type <- as.data.frame(fitsum$name)
          ...
          DOF <- as.data.frame(t(fitsum$DOF))
          ...
          if (grepl("diffusion", rownames(fitsum$CI))) {
            ...
            diffusion <- as.data.frame(t(fitsum$CI["diffusion (square meters/second)", ]))
          ...
          if (any(grepl("\\[position\\] \\(days\\)", rownames(fitsum$CI)))) {
            #if it does then extract information
            tau_p <- as.data.frame(fitsum$CI)[grepl("[position] (days)", rownames(fitsum$CI)), ]
            # Rename columns directly
            colnames(tau_p) <- c("tau_p_min_days", "tau_p_est_days", "tau_p_max_days")
            # Bind to results
            tau_p_results <- rbind(tau_p_results, tau_p)
            
          } else if (any(grepl("\\[position\\] \\(hours\\)", rownames(fitsum$CI)))) {
            tau_p <- as.data.frame(fitsum$CI)[grepl("[position] (hours)", rownames(fitsum$CI)), ]
            ...
            # if tau p is some other unit besides hours or days
          } else if (any(grepl("\\[position\\] \\((?!days|hours).+\\)", rownames(fitsum$CI), perl = TRUE))) {
            # Check for units that are not 'days' or 'hours'
            tau_p <- as.data.frame(fitsum$CI)[grepl("\\[position\\] \\((?!days|hours).+\\)", rownames(fitsum$CI), perl = TRUE), ]
            unit <- gsub(".*\\[position\\] \\((.*)\\).*", "\\1", rownames(tau_p))
            # rename columns accordingly to the extract unit text string
            colnames(tau_p) <- paste0(c("tau_p_min_", "tau_p_est_", "tau_p_max_"), unit)
            tau_p_results <- rbind(tau_p_results, tau_p)
          ...
          if (any(grepl("\\[velocity\\] \\(minutes\\)", rownames(fitsum$CI)))) {
            tau_v <- as.data.frame(fitsum$CI)[grepl("[velocity] (minutes)", rownames(fitsum$CI)), ]
            # Rename columns directly
            colnames(tau_v) <- c("tau_v_min_minutes", "tau_v_est_minutes", "tau_v_max_minutes")
            # Bind to results
            tau_v_results <- rbind(tau_v_results, tau_v)
            
          } else if (any(grepl("\\[velocity\\] \\(hours\\)", rownames(fitsum$CI)))) {
            tau_v <- as.data.frame(fitsum$CI)[grepl("[velocity] (hours)", rownames(fitsum$CI)), ]
            tau_v <- as.data.frame(fitsum$CI)[grepl("[velocity] (hours)", rownames(fitsum$CI)), ]
            ...
            # if tau v is some other unit besides minutes or hours
          } else if (any(grepl("\\[velocity\\] \\((?!minutes|hours).+\\)", rownames(fitsum$CI), perl = TRUE))) {
            # Check for units that are not 'minutes' or 'hours'
            tau_v <- as.data.frame(fitsum$CI)[grepl("\\[velocity\\] \\((?!minutes|hours).+\\)", rownames(fitsum$CI), perl = TRUE), ]
            unit <- gsub(".*\\[velocity\\] \\((.*)\\).*", "\\1", rownames(tau_v))
            # rename columns accordingly to the extract unit text string
            colnames(tau_v) <- paste0(c("tau_v_min_", "tau_v_est_", "tau_v_max_"), unit)
            tau_v_results <- rbind(tau_v_results, tau_v)
          ...
          #......................................................
          ## ballistic length scales ----
          
          # https://github.com/anagkrish/encounter_homerangeshift/blob/main/encounter_bls.R
          
          #*************************8FIND OUT WHAT THE DEFUAL UNIT ARE FORctmm summary units = FALSE*************************
          # sigma_p <- ctmm:::area.covm(FITS$sigma)
          
          # ballistic length scale: sqrt(tau_v/tau_p * sigm_p)
          # if tau p and tau v values exist (i.e values > 0) then calculate bls
          tau_p
          tau_v
          
          bls_min <- sqrt((tau_v[1,1]/tau_p[1,1]) *  sigma_p)
          bls_est <- sqrt((tau_v[1,2]/tau_p[1,2]) *  sigma_p)
          bls_max <- sqrt((tau_v[1,3]/tau_p[1,3]) *  sigma_p)
