...
#Rewritten June-July 2024

rm(list = ls())


dat3 <- read.csv('./data/input_data2/20240703_moving_window_formatted_for_tele_dat.csv')
dat <- dat3
...
window.HR <- function(data, dt, win, Fig_Path, Result_Path, UD_path) {
  ...
  for (i in seq_along(DATA)) {
    ...
    # 2. subset data in the window segment
    # for (j in seq_len(nrow(times))) {
    for (j in 1:nrow(times)) {
      ...
      # Check for data presence within the current window segment
      if (nrow(SUBSET) == 0) {
        # No data within the window, set AREAS to NA
        AREAS_lo[j] <- NA
        AREAS_ml[j] <- NA
        AREAS_hi[j] <- NA
      } else {
        # Data exists, proceed with analysis
        ...
        # Save fitted model and home range estimates
        mod_path <- file.path(Result_Path, paste0(format(start_date, "%Y-%m-%d"), "_Fits_", DATA[[i]]@info$identity, ".rda"))
        saveRDS(FIT, file = mod_path)
        
        akde_path <- file.path(UD_path, paste0(format(start_date, "%Y-%m-%d"), "_UD_", DATA[[i]]@info$identity, ".rda"))
        saveRDS(AKDE, file = akde_path)
        
        # Store AKDE results in times dataframe
        SUMMARY <- summary(AKDE, units = FALSE)
        times$AREAS_lo[k] <- SUMMARY$CI[[1]]
        times$AREAS_ml[k] <- SUMMARY$CI[[2]]
        times$AREAS_hi[k] <- SUMMARY$CI[[3]]
      }
    
    # Store results for the current ID
    results[[i]] <- list(
      identity = DATA[[i]]@info$identity,
      times = times,
...
#20240703 should be fixed now, testing on all data
...
save(results, file = "./data/input_data2/moving_window/20240703_moving_window_results.rda")
...
plot_results <- function(results, Fig_Path, data, period, index) {
  ...
  # a) Plot the relocation data, coloured by time
...
for (i in seq_along(DATA)) {
  plot_results(results[[i]], Fig_Path, DATA[[i]], period, i)
}
...
#......................................................................
# PLOT sliding window results based on a calendar scale ----
#......................................................................
...
#........................................................................
## Plot all goats on one plot via facet wrap ----
#........................................................................
...
#~~~~~~~~~~~~~PLOTTING BELOW based on within period range and herd formatting ----
...
#............................................................................
# NEED TO UPDATE BELOW...looks like its all herd related
#............................................................................
