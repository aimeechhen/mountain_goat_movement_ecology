...
# import and inspect the files
# FIT <- readRDS("./data/moving_window/fire_period/basic/fits_20250118/fits_kid_rock.rds")
# AKDE <- readRDS("./data/moving_window/fire_goats/fire_period/basic/akdes_20250118/akdes_kid_rock.rds")
# load("./data/moving_window/fire_period/basic/covariates_df_20250117/covariates_df_kid_rock.rda")


folder_path <- paste0(dir_path, "akdes_20250118")
...
# Set the folder path
folder_path <- paste0(dir_path, "fits_20250118")
# Load .shp, .tif etc files within a folder including all the subfolders
rds_files <- list.files(folder_path, pattern = "\\.rds$", full.names = TRUE)
# Import/read all the files into a list
rds_list <- lapply(rds_files, readRDS)
...
folder_path <- paste0(dir_path, "covariates_20250118")
# Load .shp, .tif etc files within a folder including all the subfolders
rds_files <- list.files(folder_path, pattern = "\\.rds$", full.names = TRUE)
# Import/read all the files into a list
rds_list <- lapply(rds_files, readRDS)
...
# Var1  Freq
# 1             area (square meters) 12828
# 2 diffusion (square meters/second)  7371
# 3            speed (meters/second)  4346
# 4                      τ (seconds)  4027
# 5               τ[decay] (seconds)    51
# 6              τ[period] (seconds)    51
# 7            τ[position] (seconds)  3293
# 8            τ[velocity] (seconds)   268
...
for (i in 1:length(AKDE)) {
  ...
  win_results$goat_name[i] <- AKDE[[i]]@info$identity
