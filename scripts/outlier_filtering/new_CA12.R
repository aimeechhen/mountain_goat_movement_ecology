...
goat$individual.local.identifier <- "CA12"
...
out_data$flag_outlier <- ifelse(out_data$speed >= 0.10, 1, 0) # 0.2 = 11, 0.15 = 28
# how many fixes were found above the plateau?
sum(out_data$flag_outlier)  #22
...
dev.off()
((good_tracks <- movement_map(good_locations, map_type = "Esri.WorldTopoMap")))
mapview::mapshot2(good_tracks, vwidth = 1600, vheight = 900,
                  file = paste0(outlie_folder, goat$individual.local.identifier[1], "_4b_map_good_tracks.png"))
...
# save progress
save(out_data_good_loc_list, file = "./data/goat/cleaning/in_progress/out_data_good_loc_list_new_final.rda")
# clean environment
rm(list = ls())
gc()
...
# # looks better, but still looks like there are more outliers
# 
# 
# 
# #........................................................
...
# plot(point_check$POI) # confirming these are the correct fixes within the bbox
# movement_map(point_check$POI) # and on the map
# # NOTE: similar point as the goatmother, leaving it in for now
# 
# # # 2c. get fix_id and record potential bad location estimates 
# # poi_id <- point_check$POI$fix_id # getting fix_id
# # # add to the list
# # potential_outliers <- c(potential_outliers, poi_id)
# # 
# # # 3c. take only updated locations (good locations + round 1 manual check)
# # updated_locations <- good_locations[!(good_locations$fix_id %in% potential_outliers), ]
# # 
# # # 4c. recheck (updated location estimates)
# # recheck <- plot_outlie(updated_locations)
# # movement_map(updated_locations)
# 
# 
# #......................................................
# # round 3
# 
# # 1d. look for outliers & plot
# # location points -> set the bounding box around the point, looks like its in down in the valley
# point_check <- plot_range(updated_locations,
#                           xmin = -2600,
#                           xmax = -2000,
#                           ymin = 1500,
...
# movement_map(point_check$data_section)
# # looks like an outlier
# # NOTE: another individual (toats_mcgoats) went down into the valley too, not removing for now
# # 
# # plot(point_check$POI) # confirming these are the correct fixes within the bbox
# # movement_map(point_check$POI) # and on the map
# 
# 
# # # 2d. get fix_id and record potential bad location estimates
# # poi_id <- point_check$POI$fix_id # getting fix_id
# # # add to the list
# # potential_outliers <- c(potential_outliers, poi_id)
# # 
# # # 3d. take only updated locations (good locations + round 1 manual check)
# # updated_locations <- good_locations[!(good_locations$fix_id %in% potential_outliers), ]
# # 
# # # 4d. recheck (updated location estimates)
# # png(paste0(outlie_folder, goat$individual.local.identifier[1], "_6_final_location_check.png"), width = 6.23*2, height = 6*1.5, units = "in", res = 600)
# # recheck <- plot_outlie(updated_locations)
# # dev.off()
# # ((final_tracks <- movement_map(updated_locations)))
# # mapview::mapshot2(final_tracks, vwidth = 1600, vheight = 900,
# #                   file = paste0(outlie_folder, goat$individual.local.identifier[1], "_7_map_final_tracks.png"))
# 
# 
...
# 
# 
# # last goat to check, save list
# save(out_data_good_loc_list, file = "./data/goat/cleaning/out_data_potential.list.rda")
# 
