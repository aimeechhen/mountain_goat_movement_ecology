...
# import data
new_collar <- readr::read_csv("data/collar_data/raw_collar/Cathedral Goat locations Sept 2023 through Feb 10 2025.csv") %>%
...
new_collar <- relocate(new_collar, c(goat_name, goat_id), .before = collar_id)


# # convert timestamp timezone
# new_collar$timestamp <- as.POSIXct(new_collar$acq_time_utc)
# new_collar$timestamp <- with_tz(new_collar$timestamp, tzone = "America/Vancouver")
# # compare time zones
# tz(new_collar$acq_time_utc)
# tz(new_collar$timestamp)
# # same value so just use the original or make a new timestamp column


# for temporal attributes
new_collar$timestamp <- as.POSIXct(new_collar$acq_time_utc)
# new_collar$date_time = as.POSIXct(new_collar$timestamp, format = '%Y-%m-%d %H:%M', tz='UTC')
