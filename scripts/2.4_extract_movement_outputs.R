...
# load("./data/movement/speed_mean.rda")
# load("./data/movement/speeds_insta.rda")
...
# compare the units being used
summary(FITS[[1]], units = FALSE) # units = FALSE, using SI units
# CI units:
# area (square meters)
# τ[position] (seconds)
# τ[velocity] (seconds)
...
# 3            speed (meters/second)   13
# 4            τ[position] (seconds)   35
# 5            τ[velocity] (seconds)   13
...
RESULTS <- data.frame(goat_id = character(length(FITS)), 
...
for (i in 1:length(FITS)) {
  ...
  tryCatch({
    # Extract id and year from names(FITS)
    # RESULTS$individual.local.identifier[i] <- names(FITS)[i]
    RESULTS$goat_id[i] <- substr(names(FITS)[i], 1, 4)
...
str(RESULTS)
RESULTS$goat_id <- as.factor(RESULTS$goat_id)
RESULTS$year <- as.factor(RESULTS$year)
# RESULTS$individual.local.identifier <- as.factor(RESULTS$individual.local.identifier)
...
# extracting movement results all at once instead of sections independently, if want to do them one at a time, go down to extract single results section
# adding results to the already built results df from above


for (i in 1:length(FITS)) {
  # for (i in 1:min(100, length(FIT))) { # testing with subset
  # extract movement model results
  ...
  if ("τ[position] (seconds)" %in% rownames(summary(FITS[[i]], units = FALSE)$CI)) {
    ...
    #..............................................
  ## tau_v
  # tau-v is the ~time it takes to cross your home range
...
movement_results <- merge(RESULTS, goat_info[, c("goat_id","goat_name", "collar_id")], by = "goat_id", all.x = TRUE)
movement_results <- dplyr::relocate(movement_results, c("goat_id","goat_name", "collar_id"), .before = year)

movement_results$goat_name <- as.factor(movement_results$goat_name)
movement_results$collar_id <- as.factor(movement_results$collar_id)

str(movement_results)

## SAVE RESULTS ----

# dir.create("./results/")
save(movement_results, file = "./results/movement_results.rda")
...
# just the hr value
meta(AKDES[grepl("2019", names(AKDES))])[1,1] # mean (km^2) low
meta(AKDES[grepl("2019", names(AKDES))])[1,2] # mean (km^2) est
meta(AKDES[grepl("2019", names(AKDES))])[1,3] # mean (km^2) high
...
save(meta_df, file = "./results/movement_meta_results.rda")
write.csv(meta_df, file = "./results/movement_meta_results.csv", row.names = FALSE)

#..........................
# alternatively, if you want it displayed across years rather than across types
meta_df2 <- data.frame(results = c("hr_km2_low", "hr_km2_est", "hr_km2_high",
...
for (i in 2019:2024) {
  ...
  meta_df2[[as.character(i)]] <- res
}

str(meta_df2)

save(meta_df2, file = "./results/movement_meta2_results.rda")
write.csv(meta_df2, file = "./results/movement_meta2_results.csv", row.names = FALSE)
...
# get results for each analysis by itself, then can combine into a single df after
...
#     summary$individual.local.identifier <- names(AKDES[i])
...
# # 35 
# 
# # change column names to indicate hr results
...
# # reinspect
# tau_p_results[grep("seconds", rownames(tau_p_results)), ] <- tau_p_results[grep("seconds", rownames(tau_p_results)), ] / 3600 # seconds into hours
# tau_p_results
# # looks much better
# 
...
# names(tau_p_results)[1] <- "tau_p_low_hour"
# names(tau_p_results)[2] <- "tau_p_est_hour"
# names(tau_p_results)[3] <- "tau_p_high_hour"
...
# tau_v_results[grep("seconds", rownames(tau_v_results)), ] <- tau_v_results[grep("seconds", rownames(tau_v_results)), ] / 60 # seconds into minutes
# # reinspect
# tau_v_results
# # looks much better
# 
