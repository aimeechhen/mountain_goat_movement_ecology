...
collar_data$goat_name[collar_data$goat_name == "kid_rock"] <- "selena_goatmez"
collar_data$goat_name[collar_data$goat_name == "rocky"] <- "goatileo"
collar_data$goat_name[collar_data$goat_name == "vertigoat"] <- "goatbriella"
collar_data$goat_name[collar_data$goat_name == "billy"] <- "ryan_goatsling"
...
goats <- c("selena_goatmez", "goatileo")
# goats <- c("selena_goatmez", "toats_mcgoats", "goatzilla", "the_goatmother", "vincent_van_goat", "goatileo")
...
# Import rasters as raster for rsf.fit() from ctmm package
elev <- raster('./data/rasters/elev_25m.tif')
dist_escape <- raster('./data/rasters/dist_escape_25m.tif')
# reproject into lat/long crs
elev <- projectRaster(elev, "EPSG:4326")
dist_escape <- projectRaster(dist_escape, "EPSG:4326")
...
dir_path <- "./data/moving_window/test" # for all goats for all data
...
for(goat in 1:length(tel_data)){
  ...
  saveRDS(rsf, file = paste0(dir_path, "rsf_20250218/rsf_", DATA@info[1], ".rds"))
