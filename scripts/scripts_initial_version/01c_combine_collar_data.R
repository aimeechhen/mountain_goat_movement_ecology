...
load("./working/data/collar_data/full_combined_data_20250505.rda")
...
# check which rows have the same values in the columns
goat_data[which(duplicated(goat_data[ , c("longitude", "latitude", "timestamp")])), ]

# check the the rows that are duplicated
goat_data[which(duplicated(goat_data[, c("longitude", "latitude", "timestamp")]) | duplicated(goat_data[, c("longitude", "latitude", "timestamp")], fromLast = TRUE)), ]
...
load("working/data/collar_data/fire_period_all_years_combined_data_20250505.rda")
