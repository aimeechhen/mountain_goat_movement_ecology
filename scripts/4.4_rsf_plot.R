...
rsf_columns <- c("rsf_elev_low", "rsf_elev_est", "rsf_elev_high", "rsf_elev_cov",
                 "rsf_esc_dist_low", "rsf_esc_dist_est", "rsf_esc_dist_high", "rsf_esc_dist_cov")
...
plot_rsf_annual_esc_dist <- 
  ggplot(data = rsf_yearly) +
  geom_hline(yintercept = 0, col = "black", linetype = "dashed") +
  geom_pointrange(aes(x = year, y = rsf_esc_dist_est, ymin = rsf_esc_dist_low, ymax = rsf_esc_dist_high, 
                      color = year), 
                  size = 0.5, position = position_dodge(width = 0.5)) +
  ylab("Escape Terrain Distance") +
  # ggtitle("b)") +
  theme_bw() +
  theme(panel.grid.major = element_blank(),
        panel.grid.minor = element_blank(),
...
plot_rsf_annual <- grid.arrange(plot_rsf_annual_elev, plot_rsf_annual_esc_dist,
                                nrow = 1)
...
plot_rsf_esc_dist <-
  ggplot(data = rsf_results) +
  geom_hline(yintercept = 0, col = "grey70", linetype = "dashed") +
  geom_pointrange(aes(x = goat_name, y = rsf_esc_dist_est, ymin = rsf_esc_dist_low, ymax = rsf_esc_dist_high, color = year),
                  size = 0.5, position = position_dodge(width = 0.5)) +
...
# 
# ggsave(plot_rsf_esc_dist,
#        file="figures/rsf/rsf_esc_dist.png",
#        width = 6.86, height = 3, units = "in", dpi = 600, bg = "transparent")
...
# plot_rsf_individual <- grid.arrange(plot_rsf_elevation, plot_rsf_esc_dist,
#                          ncol = 1)

plot_rsf_individual <- ggarrange(plot_rsf_elevation, plot_rsf_esc_dist,
                                 ncol = 1,
                                 common.legend = TRUE,
                                 legend = "top")
plot_rsf_individual
